...
function createSyncTable(context) {
    ...
    new aws_dynamodb_1.Table(stack, graphql_transformer_common_1.SyncResourceIDs.syncDataSourceID, {
        tableName,
        partitionKey: {
...
function createSyncLambdaIAMPolicy(context, stack, name, region) {
...
function syncLambdaArnResource(stackManager, name, region) {
    const substitutions = {};
    if (referencesEnv(name)) {
        Object.assign(substitutions, {
            env: stackManager.getParameter(graphql_transformer_common_1.ResourceConstants.PARAMETERS.Env),
        });
    }
